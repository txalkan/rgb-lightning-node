...
async fn run_expire_hodl_invoice_case(
    ...
    let defer_guard = if matches!(trigger, ExpiryTrigger::Time) {
        let node2_pubkey = node_info(node2_addr).await.pubkey;
        Some(defer_payment_claimable(&node2_pubkey))
    } else {
        None
    };

    let _ = send_payment_with_status(node1_addr, invoice.clone(), HTLCStatus::Pending).await;
    if defer_guard.is_some() {
        wait_for_deferred_payment().await;
        let target_ts = i128::from(
            decoded
                .timestamp
                .saturating_add(decoded.expiry_sec)
                .saturating_add(1),
        );
        let timeout = std::time::Duration::from_secs(decoded.expiry_sec.saturating_add(60));
        ...
        loop {
            ...
            if started_at.elapsed() > timeout {
                panic!(
                    "invoice expiry did not pass in time (target: {target_ts}, current: {now_ts})"
                );
            }
...
async fn claim_hodl_invoice_btc_rgb() {
    ...
    // hold the claim after Claiming is persisted so competing requests deterministically observe
    // the settlement-in-progress state.
    let payee_pubkey = node_info(node2_addr).await.pubkey;
    let hold_claim_guard = hold_hodl_claim(&payee_pubkey);
    let claim_node = node2_addr;
    let claim_hash = payment_hash.clone();
    let claim_preimage = preimage.clone();
    let claim_task = tokio::spawn(async move {
        claim_hodl_invoice(claim_node, claim_hash, claim_preimage).await;
    });
    wait_for_held_hodl_claim().await;

    // reject cancellation while settlement is deterministically held in progress
    invoice_cancel_expect_error(
        node2_addr,
        payment_hash.clone(),
        StatusCode::FORBIDDEN,
    ...
    // a second claim must also be rejected while settlement is in progress
    invoice_claim_expect_error(
        node2_addr,
        payment_hash.clone(),
        preimage.clone(),
