...
use crate::ldk::{
    DEFER_PAYMENT_CLAIMABLE_ON_NODE, FAIL_HODL_CLAIM_ON_NODE, FEE_RATE,
    FORCE_PUSH_ASSET_AMOUNT_ON_NODE, HELD_PAYMENT_CLAIMABLE_COUNT, HODL_CLAIM_HELD,
    HOLD_HODL_CLAIM_ON_NODE, HOLD_PAYMENT_CLAIMABLE_ON_NODE, IGNORE_INBOUND_CHANNELS_ON_NODE,
...
use crate::routes::{
    ...
    DecodeSwapstringRequest, DecodeSwapstringResponse, DisconnectPeerRequest, EmptyResponse,
    FailTransfersRequest, FailTransfersResponse, GetAssetMediaRequest, GetAssetMediaResponse,
    GetChannelIdRequest, GetChannelIdResponse, GetConsignmentRequest, GetConsignmentResponse,
    ...
    LNInvoiceResponse, LdkChainSync, ListAssetsRequest, ListAssetsResponse, ListChannelsResponse,
    ...
    PaymentType, Peer, PostAssetMediaResponse, ProvideOutOfBandAckRequest,
...
fn hold_hodl_claim(payee_pubkey: &str) -> NodeOverrideGuard {
    HODL_CLAIM_HELD.store(false, Ordering::SeqCst);
    NodeOverrideGuard::set(&HOLD_HODL_CLAIM_ON_NODE, payee_pubkey)
}

async fn wait_for_held_hodl_claim() {
    let t_0 = OffsetDateTime::now_utc();
    while !HODL_CLAIM_HELD.load(Ordering::SeqCst) {
        if (OffsetDateTime::now_utc() - t_0).as_seconds_f32() > 40.0 {
            panic!("HODL claim was not held");
        }
        tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    }
}
...
async fn ln_invoice(
    ...
    ln_invoice_with_optional_payment_hash(
        node_address,
        amt_msat,
        asset_id,
        asset_amount,
...
async fn ln_invoice_with_optional_payment_hash(
    node_address: SocketAddr,
    amt_msat: Option<u64>,
    asset_id: Option<&str>,
    asset_amount: Option<u64>,
    ...
    if let Some(payment_hash) = payment_hash.as_ref() {
        println!(
            "generating HODL invoice for {asset_amount:?} of asset {asset_id:?} for node {node_address}, payment hash: {payment_hash:?}"
        );
    } else {
        println!(
